...
const Index = () => {
  ...
  const handleBrainDump = async (text: string) => {
    ...
    try {
      ...
      const mockResult: DumpResult = {
        actionPlan: [
          "Break down that big project into 3 smaller chunks you can tackle today",
          "Set a 25-minute timer and work on just one chunk (Pomodoro technique)",
          "Schedule 15 minutes tomorrow to plan your week and reduce future stress",
          "Reach out to one person who might help you with this challenge"
        ],
        wellnessReset: "Take 5 deep breaths: in for 4 counts, hold for 4, out for 6. This activates your parasympathetic nervous system and reduces cortisol. Try it right now! 🫁",
        motivationBoost: "You're handling more than most people could manage, and you're still here asking for help. That's not weakness - that's wisdom. Every expert was once a beginner who refused to give up."
      };
      ...
      // Add mood entry
      const emotion = text.toLowerCase().includes('stress') ? 'stressed' : 
                     text.toLowerCase().includes('anxious') ? 'anxious' :
                     text.toLowerCase().includes('tired') ? 'tired' : 'calm';
      const intensity = Math.ceil(text.length / 50); // Simple intensity based on text length
        ...
        {!showResult ? (
          <>
            ...
            <section className="text-center space-y-4">
              ...
              <BrainDumpInput onSubmit={handleBrainDump} isLoading={isLoading} />
            </section>
